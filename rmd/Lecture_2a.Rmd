...
Monte Carlo methods are ones that involve repeated random sampling to estimate unknown parameters. 

<!--more-->

### Monte Carlo Integration

Monte Carlo integration is a numerical technique based on distributions and randon numbers.  Unlike Riemann Sums, Monte Carlo integration choose points randomly within the bound of interest and assumes the point frequency corresponds to the "height" at that point in the interval.

As an example, consider the function and integral:

\begin{equation}
H(x,y) = 1 if x^2+y^2 \le 1, 0\text{ otherwise}
\end{equation}
...
which we know integrates to pi.  We could evaluate that integral by:

\begin{equation}
\pi = 4*\frac{1}{N}\Sigma_{i=1}^N H(x_i,y_i)
\end{equation}
where ($x_i$,$y_i$) are drawn from unif[0,1].

----

### Estimate $\pi$ using darts

We will come back to the above, for now, we will simply and say Monte Carlo methods are ones that involve repeated random sampling to estimate unknown parameters or distributions.  As an example, we can use repeated random sampling from uniform distributions to estimate pi.  Consider playing darts where we have inscribed a circle within the area given by a dart board.  If the circle and square are centered at c(0,0) where the circle has radius r=1 while the square has sides of length d=2*r, we can calculate the probablity of hitting the circle vs the square as the ratio of areas given by:
...
plot(x=x.pos,y=y.pos,xlim=c(0,10),ylim=c(0,10))
# draw the lines
abline(v=seq(1,9,1))
# draw the needles
